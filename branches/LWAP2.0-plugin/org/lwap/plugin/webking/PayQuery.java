...
//		// 从form通过get()方法取值
		...
		ClientQueryPayUtils queryPayUtils = new ClientQueryPayUtils(ip,port,true);
		EBHeader header = EBHeaderUtils.createHeader("MBTS",
			...
			if(null != ebe){
				...
				for (int i =0 ;i<paydetail.length;i++){
					...
					record.put("DETAILSEQID", paydetail[i].getDetailSeqID().toString());
					record.put("BIZNO", paydetail[i].getDetailBizNo().toString());
					record.put("PAYEEACCNO", paydetail[i].getPayeeAccNo().toString());
					record.put("PAYEEACCNAME", paydetail[i].getPayeeAccName().toString());
					...
					String country =paydetail[i].getPayeeCountry()==null?"":paydetail[i].getPayeeCountry().toString();
					record.put("COUNTRY", country);
					String province =paydetail[i].getPayeeProvince()==null?"":paydetail[i].getPayeeProvince().toString();
					record.put("PROVINCE", province);
					String city =paydetail[i].getPayeeCity()==null?"":paydetail[i].getPayeeCity().toString();
					record.put("CITY", city);
					String areacode =paydetail[i].getPayeeAreaCode()==null?"":paydetail[i].getPayeeAreaCode().toString();
					record.put("AREACODE", areacode);
					String cnpscode =paydetail[i].getPayeeCnapsCode()==null?"":paydetail[i].getPayeeCnapsCode().toString();
