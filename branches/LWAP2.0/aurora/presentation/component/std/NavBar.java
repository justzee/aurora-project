...
public class NavBar extends ToolBar {
	...
	public void onCreateViewContent(BuildSession session, ViewContext context) throws IOException{
		try {
			...
			view.addChild(createButton("nav-firstpage","background-position:0px 1px;","function(){$('"+dataset+"').firstPage()}","绗竴椤�"));
			view.addChild(createButton("nav-prepage","background-position:0px -31px;","function(){$('"+dataset+"').prePage()}","涓婁竴椤�"));
			
			CompositeLoader loader = new CompositeLoader();
			String pagetext = "<div class='item-label' style='margin-left:2px;margin-right:2px;'>椤垫暟:</div>";
			...
			String text = "<div id='"+pageId+"' class='item-label' style='margin-left:5px;margin-right:5px;'>鍏��/div>";
			CompositeMap totalpage = loader.loadFromString(text,"UTF-8");
			view.addChild(totalpage);
			
			view.addChild(createButton("nav-nextpage","background-position:0px -46px;","function(){$('"+dataset+"').nextPage()}","涓嬩竴椤�"));
			view.addChild(createButton("nav-lastpage","background-position:0px -15px","function(){$('"+dataset+"').lastPage()}","鏈�鍚庨〉"));
			view.addChild(createButton("nav-refresh","background-position:0px -63px;","function(){$('"+dataset+"').query($('"+dataset+"').currentPage)}","鍒锋柊"));
