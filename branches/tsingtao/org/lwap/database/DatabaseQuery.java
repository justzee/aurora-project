...
public class DatabaseQuery extends DatabaseAccess {
    ...
    public static DatabaseQuery createPagedQuery(String sql, String elm,
            String pagesize_param_name, String pagenum_param_name) {
        DatabaseQuery query = new DatabaseQuery(sql, elm);
        query.setPageResultset(true);
    ...
    public static DatabaseQuery createPagedQuery(String sql, String elm) {
        return createPagedQuery(sql, elm, DatabaseQuery.DEFAULT_PAGESIZE_PARAM,
                DatabaseQuery.DEFAULT_PAGENUM_PARAM);
    }
    ...
    public void execute(Connection conn, CompositeMap parameter,
        ...
        if (_sql == null)
            throw new IllegalArgumentException(
                    "'Sql' missing for query statement");
        ...
        try {
            ...
            // end check
            	...
            	ResultSetToFile resultSetToFile=new ResultSetToFile();              	
            	try {
            		String path= resultSetToFile.write(rs);
                	if(path!=null){
                		CompositeMap obj=target.createChild(this.getTarget());
                		obj.put("type", "file");
                		obj.createChild("FilePath").put("VALUE",path);
                	}
				} catch (Exception e) {					
                ...
                if (fetch_one) {
                    ...
                    if (isPageResultset()) {
                        // Always get pagesize and pagenum from 'PageSize',
                        // 'PageNum' attribute first
                        // If not specified, see if can get pagesize & pagenum
                        // by alternaltive parameter name
                        long page_size = this.getPagesize();
                        if (page_size < 0)
                            page_size = getLong(parameter,
                                    this.getPagesizeParamName(), 1);

                        long page_num = this.getPageNum();
                        if (page_num < 0)
                            page_num = getLong(parameter,
                                    this.getPageNumParamName(), 1);

                        mLogger.log(Level.CONFIG, "page_size=" + page_size
                                + " page_num=" + page_num);
                ...
                CompositeMap transform_conf = getObjectContext().getChild(
                        KEY_TRANSFORM_LIST);
                if (transform_conf != null)
                    target_map = Transformer.doBatchTransform(target_map,
                            transform_conf.getChilds());
    ...
    public String getPageNumParamName() {
        return getString(DatabaseQuery.KEY_PAGENUM_PARAM_NAME,
                DatabaseQuery.DEFAULT_PAGENUM_PARAM);
    }
    ...
    public String getPagesizeParamName() {
        return getString(DatabaseQuery.KEY_PAGESIZE_PARAM_NAME,
                DEFAULT_PAGESIZE_PARAM);
    }
