...
//		appendRequestParam( request, url, ExcelExportService);		
	...
	void printTableData( CompositeMap item, int row )
	throws IOException {
	   if( getColumns() == null) return;
	   int col = 0;
	   //while( it.hasNext()){		   
		...
		if( model.getChilds() != null ){ 
			...
			if(model.get("type")==null){
				while( it.hasNext()){
					...
					printTableData((CompositeMap)it.next(), row+1);
					row++;
				}
			}else{				
				File f=new File(((CompositeMap)it.next()).getString("VALUE"));
	...
	//				System.out.print();
					for(String line  =  reader.readLine(); line != null; line  = reader.readLine())   { 
						...
						for(int i=0;i<arry.length;i++){
							...
							record.put(key.substring(0,index), key.substring(index+1));						
						}	
						printTableData(record, row+1);
						row++;
					}
				}finally{
					if(reader!=null)
