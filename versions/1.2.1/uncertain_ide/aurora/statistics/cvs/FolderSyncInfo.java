...
/**
 * Value (immutable) object that represents workspace state information about
 * the contents of a folder that was retreived from a CVS repository. It is a
 * specialized representation of the files from the CVS sub-directory that
 * contain folder specific connection information (e.g. Root, Repository, Tag).
 * 
 */
public class FolderSyncInfo {
	...
	// The Repository value for virtual directories (i.e. local with no
	...
	// if true then it means only part of the folder was fetched from the
	// repository, and CVS will not create
	 ...
	 * 
	 * @param repo
	 *            the relative path of this folder in the repository, cannot be
	 *            <code>null</code>.
	 * @param root
	 *            the location of the repository, cannot be <code>null</code>.
	 * @param tag
	 *            the tag set for the folder or <code>null</code> if there is no
	 *            tag applied.
	 * @param isStatic
	 *            to indicate is only part of the folder was fetched from the
	 *            server.
	 */
	public FolderSyncInfo(String repo, String root, CVSTag tag, boolean isStatic) {

		this.repository = repo;
		// intern the root so that caching of FolderSyncInfos for folders will
		// use less space
	...
	private void ensureRepositoryRelativeToRoot() {
		...
		if (repository.startsWith(SEPARATOR)) {
			repository = repository.substring(SEPARATOR.length());
		}
	}
	...
	public boolean equals(Object other) {
		...
		if ((getTag() == null) || (syncInfo.getTag() == null)) {
			if ((getTag() == null) && (syncInfo.getTag() != null)
					&& (syncInfo.getTag().getType() != CVSTag.HEAD)) {
				return false;
			} else if ((syncInfo.getTag() == null) && (getTag() != null)
					&& (getTag().getType() != CVSTag.HEAD)) {
				return false;
			}
	...
	/**
	 * Answer the directory portion of the root. For example, if root =
	 * :pserver:user@host:/home/user/repo then /home/user/repo is return.
	 * <p>
	 * The root does not neccesarily contain a user name, in which cas the
	 * format is :pserver:host:/home/user/repo.
	 * 
	 * 
	 * @return String
	 */
	 ...
	 * the repository to the repository location speficied in the root.
	 * 
	 * Example: root = :pserver:user@host:/home/user/repo repository =
	 * folder1/folder2
	 * 
	 * Returns: /home/users/repo/folder1/folder2
	 * 
	 ...
	 * between the root directory and the rest of the path. For example: root =
	 * :pserver:user@host:/home/user/repo/ repository = folder1/folder2 must
	 * return: /home/users/repo//folder1/folder2
	 * 
	 * @return the full path of this folder on the server.
	 * @throws a
	 *             CVSException if the root or repository is malformed.
	 */
	...
	/*
	 * Provide a hashCode() method that gaurentees that equal object will have
	 * the same hashCode
	 */
	public int hashCode() {
	...
	public boolean isSameMapping(FolderSyncInfo other) {
		if (other == null)
			return false;
		return (this.getRoot().equals(other.getRoot()) && this.getRepository()
				.equals(other.getRepository()));
	}

	/**
	 * Convert a FolderSyncInfo into a byte array that can be stored in the
	...
	/**
	 * Convert a byte array that was created using getBytes(FolderSyncInfo) into
	 * a FolderSyncInfo
	 */
