...
(function() {
	var idSeed = 0, ua = navigator.userAgent.toLowerCase(), check = function(r) {
		return r.test(ua);
			...
			&& check(/msie 7/), isIE8 = isIE && check(/msie 8/), isIE6 = isIE
			&& !isIE7 && !isIE8, isGecko = !isWebKit && check(/gecko/), isGecko3 = isGecko
	...
	Ext.apply(Ext, {
		...
		id : function(el, prefix) {
			return (el = Ext.getDom(el) || {}).id = el.id
					|| (prefix || "ext-gen") + (++idSeed);
		},
...
(function() {
	...
	(function() {
		var doc = document, isCSS1 = doc.compatMode == "CSS1Compat", MAX = Math.max, PARSEINT = parseInt;
	...
	Ext.lib.Event = function() {
		var loadComplete = false, listeners = [], unloadListeners = [], retryCount = 0, onAvailStack = [], _interval, locked = false, win = window, doc = document,

		// constants
		POLL_RETRYS = 200, POLL_INTERVAL = 20, EL = 0, TYPE = 1, FN = 2, WFN = 3, OBJ = 3, ADJ_SCOPE = 4,
		// private
		doAdd = function() {
			var ret;
		...
		function _tryPreloadAttach() {
			var ret = false, notAvail = [], element, tryAgain = !loadComplete
					|| (retryCount > 0);
				...
				Ext.each(onAvailStack, function(v, i, a) {
							if (v && (element = doc.getElementById(v.id))) {
								if (!v.checkReady || loadComplete
										|| element.nextSibling
										|| (doc && doc.body)) {
									element = v.override ? (v.override === true
											? v.obj
											: v.override) : element;
		...
		var pub = {
			...
			addListener : function(el, eventName, fn) {
				...
				if (el && fn) {
					if ("unload" == eventName) {
						ret = !!(unloadListeners[unloadListeners.length] = [el,
								eventName, fn]);
					} else {
						listeners.push([el, eventName, fn,
								ret = doAdd(el, eventName, fn, false)]);
					}
				}
				return !!ret;
			},
			...
			getListeners : function(el, eventName) {
				var me = this, results = [], searchLists;

				if (eventName) {
					searchLists = eventName == 'unload'
							? unloadListeners
							: listeners;
				} else {
			...
			_unload : function(e) {
				var EU = Ext.lib.Event, i, j, l, len, index, scope;
	...
	Ext.lib.Ajax = function() {
		...
		function createResponseObject(o, callbackArg) {
			var headerObj = {}, headerStr, conn = o.conn;
		...
		function asyncRequest(method, uri, callback, postData) {
			...
			if (o) {
				...
				if (postData && pub.useDefaultHeader
						&& (!pub.headers || !pub.headers['Content-Type'])) {
		...
		var pub = {
			request : function(method, uri, cb, data, options) {
				if (options) {
					var me = this, xmlData = options.xmlData, jsonData = options.jsonData;

					Ext.applyIf(me, options);

					if (xmlData || jsonData) {
						initHeader('Content-Type', xmlData
										? 'text/xml'
										: 'application/json');
						data = xmlData
								|| (Ext.isObject(jsonData) ? Ext
	...
	(function() {
		...
		(function() {
			...
			var superclass = EXTLIB.Motion.superclass, proto = EXTLIB.Motion.prototype, pointsRe = /^points$/i;
		...
		Ext.apply(EXTLIB.ColorAnim.prototype, {
			...
			doMethod : function(attr, start, end) {
				...
				if (colorRE.test(attr)) {
					...
					Ext.each(start, function(v, i) {
								val[i] = superclass.doMethod.call(me, attr, v,
										end[i]);
							});
...
(function() {
	...
	var EXTUTIL = Ext.util, TOARRAY = Ext.toArray, EACH = Ext.each, ISOBJECT = Ext.isObject, TRUE = true, FALSE = false;
	...
	EXTUTIL.Observable.prototype = function() {
		var filterOptRe = /^(?:scope|delay|buffer|single)$/, toLower = function(
				s) {
		...
		return {
			...
			fireEvent : function() {
				var a = TOARRAY(arguments), ename = toLower(a[0]), me = this, ret = TRUE, ce = me.events[ename], q, c;
			...
			addListener : function(eventName, fn, scope, o) {
				...
				if (ISOBJECT(eventName)) {
					...
					if (typeof ce == "boolean") {
						me.events[eventName] = ce = new EXTUTIL.Event(me,
								eventName);
					}
	...
	EXTUTIL.Event.prototype = {
		addListener : function(fn, scope, options) {
			...
			if (!me.isListening(fn, scope)) {
				...
				if (me.firing) { // if we are currently firing this event,
									// don't disturb the listener loop
		...
		fire : function() {
			var me = this, args = TOARRAY(arguments), ret = TRUE;
...
Ext.DomHelper = function() {
	...
	afterbegin = "afterbegin", afterend = "afterend", beforebegin = "beforebegin", beforeend = "beforeend", ts = '<table>', te = '</table>', tbs = ts
	...
	function createHtml(o) {
		...
		if (typeof o == 'string') {
			...
			// Now either just close the tag or try to add children and close
			// the tag.
	...
	function insertIntoTable(tag, where, el, html) {
		...
		if (tag == 'td' && (where == afterbegin || where == beforeend)
				|| !/td|tr|tbody/i.test(tag)
				&& (where == beforebegin || where == afterend)) {
			return;
		}
		before = where == beforebegin ? el : where == afterend
				? el.nextSibling
	...
	pub = {
		...
		insertHtml : function(where, el, html) {
			...
			// add these here because they are used in both branches of the
			// condition.
			...
			if (el.insertAdjacentHTML) {
				if (tableRe.test(el.tagName)
						&& (rs = insertIntoTable(el.tagName.toLowerCase(),
								where, el, html))) {
					return rs;
				}
				...
				if (hash[where]) {
					...
					el.parentNode.insertBefore(frag, where == beforebegin
									? el
									: el.nextSibling);
					return el[(where == beforebegin ? "previous" : "next")
							+ "Sibling"];
				} else {
					...
					if (el.firstChild) {
						...
						frag = range.createContextualFragment(html);
						where == afterbegin ? el.insertBefore(frag,
								el.firstChild) : el.appendChild(frag);
					} else {
						el.innerHTML = html;
					}
					return el[rangeEl];
...
Ext.Template.prototype = {
	...
	applyTemplate : function(values) {
		...
		return me.compiled ? me.compiled(values) : me.html.replace(me.re,
				function(m, name) {
...
Ext.DomQuery = function() {
	var cache = {}, simpleCache = {}, valueCache = {}, nonSpace = /\S/, trimRe = /^\s+|\s+$/g, tplRe = /\{(\d+)\}/g, modeRe = /^(\s?[\/>+~]\s?|\s|$)/, tagTokenRe = /^(#)?([\w-\*]+)/, nthRe = /(\d*)n\+?(\d*)/, nthRe2 = /\D/,
	// This is for IE MSXML which does not support expandos.
	// IE runs the same speed using setAttribute, however FF slows way down
	// and Safari completely fails so they need to continue to use expandos.
	isIE = window.ActiveXObject ? true : false, isOpera = Ext.isOpera, key = 30803;
	...
	return {
		...
		compile : function(path, type) {
			...
			var fn = ["var f = function(root){\n var mode; ++batch; var n = root || document;\n"], q = path, mode, lq, tk = Ext.DomQuery.matchers, tklen = tk.length, mm,
...
Ext.EventManager = function() {
	var docReadyEvent, docReadyProcId, docReadyState = false, E = Ext.lib.Event, D = Ext.lib.Dom, DOC = document, WINDOW = window, IEDEFERED = "ie-deferred-loader", DOMCONTENTLOADED = "DOMContentLoaded", elHash = {}, propRe = /^(?:scope|delay|buffer|single|stopEvent|preventDefault|stopPropagation|normalized|args|delegate)$/;

	// / There is some jquery work around stuff here that isn't needed in Ext
	// Core.
	function addListener(el, ename, fn, wrap, scope) {
		var id = Ext.id(el), es = elHash[id] = elHash[id] || {};
		...
		if (ename == "mousewheel" && el.addEventListener) { // workaround for
															// jQuery
		...
		if (ename == "mousedown" && el == document) { // fix stopped
														// mousedowns on the
														// document
	...
	function initDocReady() {
		...
		if (Ext.isGecko || Ext.isOpera) {
			...
			DOC.write("<s" + 'cript id=' + IEDEFERED + ' defer="defer" src="/'
					+ '/:"></s' + "cript>");
	...
	function listen(element, ename, opt, fn, scope) {
		var o = !Ext.isObject(opt) ? {} : opt, el = Ext.getDom(element);
	...
	var pub = {
		...
		removeListener : function(element, eventName, fn, scope) {
			var el = Ext.getDom(element), id = Ext.id(el), wrap;

			Ext.each((elHash[id] || {})[eventName], function(v, i, a) {
						if (Ext.isArray(v) && v[0] == fn
								&& (!scope || v[2] == scope)) {
			...
			if (eventName == "mousedown" && el == DOC && wrap) { // fix
																	// stopped
																	// mousedowns
																	// on the
																	// document
		...
		removeAll : function(el) {
			var id = Ext.id(el = Ext.getDom(el)), es = elHash[id], ename;
...
(function() {
	...
	var initExtCss = function() {
		...
		if (Ext.isStrict) { // add to the parent to allow for selectors like
							// ".ext-strict .ext-ie"
			var p = bd.parentNode;
			if (p) {
...
Ext.EventObject = function() {
	...
	Ext.EventObjectImpl.prototype = {
		...
		setEvent : function(e) {
			...
			if (e) {
				// normalize buttons
				me.button = e.button ? btnMap[e.button] : (e.which ? e.which
						- 1 : -1);
		...
		within : function(el, related, allowEl) {
			if (el) {
				...
				return t
						&& ((allowEl ? (t == Ext.getDom(el)) : false) || Ext
								.fly(el).contains(t));
			}
			return false;
		}
...
(function() {
	...
	Ext.Element = function(element, forceNew) {
		...
		if (!forceNew && id && Ext.Element.cache[id]) { // element object
														// already exists
	...
	var D = Ext.lib.Dom, DH = Ext.DomHelper, E = Ext.lib.Event, A = Ext.lib.Anim, El = Ext.Element;

	El.prototype = {

		set : function(o, useSet) {
			var el = this.dom, attr, val;
		...
		contains : function(el) {
			return !el ? false : Ext.lib.Dom.isAncestor(this.dom, el.dom
							? el.dom
							: el);
		},
	...
	El.get = function(el) {
		...
		if (typeof el == "string") { // element id
			...
			if (el != docEl) {
				...
				El.cache[el.id] = el; // in case it was created directly with
	...
	function garbageCollect() {
		if (!Ext.enableGarbageCollector) {
			...
			for (eid in El.cache) {
				...
				// -------------------------------------------------------
				if (!d || !d.parentNode
						|| (!d.offsetParent && !DOC.getElementById(eid))) {
					delete El.cache[eid];
	...
	El.fly = function(el, named) {
		...
		if (el = Ext.getDom(el)) {
			(El._flyweights[named] = El._flyweights[named]
...
Ext.Element.addMethods(function() {
	...
	return {
		...
		findParent : function(simpleSelector, maxDepth, returnEl) {
			...
			if (Ext.isGecko
					&& Object.prototype.toString.call(p) == '[object XULElement]') {
				return null;
			}
...
Ext.Element.addMethods(function() {
	var GETDOM = Ext.getDom, GET = Ext.get, DH = Ext.DomHelper, isEl = function(
			el) {
	...
	return {
		...
		replaceWith : function(el) {
			var me = this, Element = Ext.Element;
			if (isEl(el)) {
				el = GETDOM(el);
		...
		createChild : function(config, insertBefore, returnDom) {
			...
			return insertBefore ? DH.insertBefore(insertBefore, config,
					returnDom !== true) : DH[!this.dom.firstChild
					? 'overwrite'
...
Ext.Element.addMethods(function() {
	...
	var propCache = {}, camelRe = /(-[a-z])/gi, classReCache = {}, view = document.defaultView, propFloat = Ext.isIE
			? 'styleFloat'
			: 'cssFloat', opacityRe = /alpha\(opacity=(.*)\)/i, trimRe = /^\s+|\s+$/g, EL = Ext.Element, PADDING = "padding", MARGIN = "margin", BORDER = "border", LEFT = "-left", RIGHT = "-right", TOP = "-top", BOTTOM = "-bottom", WIDTH = "-width",
	...
	function chkCache(prop) {
		return propCache[prop]
				|| (propCache[prop] = prop == 'float' ? propFloat : prop
	...
	return {
		...
		adjustWidth : function(width) {
			var me = this;
			if (typeof width == "number" && me.autoBoxAdjust
					&& !me.isBorderBox()) {
		...
		adjustHeight : function(height) {
			var me = this;
			if (typeof height == "number" && me.autoBoxAdjust
					&& !me.isBorderBox()) {
		...
		addClass : function(className) {
			...
			Ext.each(className, function(v) {
						me.dom.className += (!me.hasClass(v) && v
								? " " + v
								: "");
					});
			return me;
		...
		removeClass : function(className) {
			...
			if (me.dom.className) {
				Ext.each(className, function(v) {
							me.dom.className = me.dom.className
									.replace(
											classReCache[v] = classReCache[v]
													|| new RegExp(
															'(?:^|\\s+)'
																	+ v
																	+ '(?:\\s+|$)',
		...
		getStyle : function() {
			return view && view.getComputedStyle ? function(prop) {
				...
				if (el == document)
					return null;
				prop = chkCache(prop);
				return (v = el.style[prop]) ? v : (cs = view.getComputedStyle(
						el, "")) ? cs[prop] : null;
			} : function(prop) {
				...
				return el.style[prop]
						|| ((cs = el.currentStyle) ? cs[prop] : null);
			};
		}(),

		getColor : function(attr, defaultValue, prefix) {
			var h, v = this.getStyle(attr), color = prefix || "#";
			...
			if (!animate || !me.anim) {
				if (Ext.isIE) {
					var opac = opacity < 1 ? 'alpha(opacity=' + opacity * 100
							+ ')' : '', val = s.filter.replace(opacityRe, '')
							.replace(trimRe, '');
		...
		clearOpacity : function() {
			...
			if (Ext.isIE) {
				if (!Ext.isEmpty(style.filter)) {
					style.filter = style.filter.replace(opacityRe, '').replace(
							trimRe, '');
				}
			} else {
		...
		getHeight : function(contentHeight) {
			...
			h = !contentHeight ? h : h - this.getBorderWidth("tb")
					- this.getPadding("tb");
			return h < 0 ? 0 : h;
		},
		...
		getWidth : function(contentWidth) {
			...
			w = !contentWidth ? w : w - this.getBorderWidth("lr")
					- this.getPadding("lr");
			return w < 0 ? 0 : w;
		},
...
(function() {
	...
	Ext.Element.addMethods({
		...
		setX : function(x, animate) {
			return this.setXY([x, this.getY()], animTest.call(this, arguments,
							animate, 1));
		},

		setY : function(y, animate) {
			return this.setXY([this.getX(), y], animTest.call(this, arguments,
							animate, 1));
		},
		...
		setLocation : function(x, y, animate) {
			return this.setXY([x, y], animTest
							.call(this, arguments, animate, 2));
		},
		...
		getLeft : function(local) {
			return !local ? this.getX() : parseInt(this.getStyle(LEFT), 10)
					|| 0;
		},

		getRight : function(local) {
			var me = this;
			return !local ? me.getX() + me.getWidth() : (me.getLeft(true) + me
					.getWidth())
					|| 0;
		},
		...
		getBottom : function(local) {
			var me = this;
			return !local ? me.getY() + me.getHeight() : (me.getTop(true) + me
					.getHeight())
					|| 0;
		},
		...
		translatePoints : function(x, y) {
			y = isNaN(x[1]) ? y : x[1];
			x = isNaN(x[0]) ? x : x[0];
			var me = this, relative = me.isStyle(POSITION, RELATIVE), o = me
					.getXY(), l = parseInt(me.getStyle(LEFT), 10), t = parseInt(
					me.getStyle(TOP), 10);
...
Ext.Element.addMethods(function() {
	var VISIBILITY = "visibility", DISPLAY = "display", HIDDEN = "hidden", NONE = "none", ORIGINALDISPLAY = 'originalDisplay', VISMODE = 'visibilityMode', ELDISPLAY = Ext.Element.DISPLAY, data = Ext.Element.data, getDisplay = function(
			dom) {
	...
	return {
		...
		anim : function(args, opt, animType, defaultDur, defaultEase, cb) {
			...
			opt = opt || {};
			var me = this, anim = Ext.lib.Anim[animType](me.dom, args,
		...
		isVisible : function() {
			return !this.isStyle(VISIBILITY, HIDDEN)
					&& !this.isStyle(DISPLAY, NONE);
		},

		setVisible : function(visible, animate) {
			var me = this, dom = me.dom, isDisplay = getVisMode(this.dom) == ELDISPLAY;
			...
			if (!animate || !me.anim) {
				...
				me.anim({
						...
						function() {
							if (!visible) {
								dom.style[isDisplay ? DISPLAY : VISIBILITY] = (isDisplay)
										? NONE
										: HIDDEN;
		...
		fixDisplay : function() {
			...
			if (me.isStyle(DISPLAY, NONE)) {
				...
				me.setStyle(DISPLAY, getDisplay(this.dom)); // first try
															// reverting to
															// default
				if (me.isStyle(DISPLAY, NONE)) { // if that fails, default to
													// block
...
(function() {
	// contants
	var NULL = null, UNDEFINED = undefined, TRUE = true, FALSE = false, SETX = "setX", SETY = "setY", SETXY = "setXY", LEFT = "left", BOTTOM = "bottom", TOP = "top", RIGHT = "right", HEIGHT = "height", WIDTH = "width", POINTS = "points", HIDDEN = "hidden", ABSOLUTE = "absolute", VISIBLE = "visible", MOTION = "motion", POSITION = "position", EASEOUT = "easeOut",

	flyEl = new Ext.Element.Flyweight(), queues = {}, getObject = function(o) {
		return o || {};
	}, fly = function(dom) {
		flyEl.dom = dom;
		flyEl.id = Ext.id(dom);
	...
	Ext.Fx = {
		...
		// private - calls the function taking arguments from the argHash based
		// on the key. Returns the return value of the function.
		...
		slideIn : function(anchor, o) {
			...
			me.queueFx(o, function() {
						...
						function argCalc(wrap, style, ww, wh, sXY, sXYval, s1,
								s2, w, h, p) {
							var ret = {};
						...
						args = fly(dom).switchStatements(anchor.toLowerCase(),
								argCalc, {
									t : [wrap, st, b.width, 0, NULL, NULL,
											LEFT, BOTTOM, NULL, bh, NULL],
									l : [wrap, st, 0, b.height, NULL, NULL,
											RIGHT, TOP, bw, NULL, NULL],
									r : [wrap, st, b.width, b.height, SETX,
											b.right, LEFT, TOP, NULL, NULL, pt],
									b : [wrap, st, b.width, b.height, SETY,
											b.bottom, LEFT, TOP, NULL, bh, pt],
									tl : [wrap, st, 0, 0, NULL, NULL, RIGHT,
											BOTTOM, bw, bh, pt],
									bl : [wrap, st, 0, 0, SETY, b.y + b.height,
											RIGHT, TOP, bw, bh, pt],
									br : [wrap, st, 0, 0, SETXY,
											[b.right, b.bottom], LEFT, TOP, bw,
											bh, pt],
									tr : [wrap, st, 0, 0, SETX, b.x + b.width,
											LEFT, BOTTOM, bw, bh, pt]
								});

						st.visibility = VISIBLE;
						fly(wrap).show();

						arguments.callee.anim = fly(wrap).fxanim(args, o,
								MOTION, .5, EASEOUT, after);
					});
			return me;
		},
		...
		slideOut : function(anchor, o) {
			o = getObject(o);
			var me = this, dom = me.dom, st = dom.style, xy = me.getXY(), wrap, r, b, a, zero = {
				to : 0
			};
			...
			me.queueFx(o, function() {
						...
						a = fly(dom).switchStatements(anchor.toLowerCase(),
								argCalc, {
									...
									tl : [st, RIGHT, BOTTOM, WIDTH, zero,
											HEIGHT, zero],
									bl : [st, RIGHT, TOP, WIDTH, zero, HEIGHT,
											zero, POINTS, {
												to : [b.x, b.bottom]
											}],
									br : [st, LEFT, TOP, WIDTH, zero, HEIGHT,
											zero, POINTS, {
												to : [b.x + b.width, b.bottom]
											}],
									tr : [st, LEFT, BOTTOM, WIDTH, zero,
											HEIGHT, zero, POINTS, {
												to : [b.right, b.y]
											}]
								});

						arguments.callee.anim = fly(wrap).fxanim(a, o, MOTION,
								.5, EASEOUT, after);
					});
			return me;
		},
		...
		highlight : function(color, o) {
			...
			me.queueFx(o, function() {
						...
						a[attr] = {
							from : color || "ffff9c",
							to : o.endColor || fly(dom).getColor(attr)
									|| "ffffff"
						};
						arguments.callee.anim = fly(dom).fxanim(a, o, 'color',
								1, 'easeIn', after);
					});
			return me;
		},
		...
		frame : function(color, count, o) {
			...
			me.queueFx(o, function() {
				...
				var xy = fly(dom).getXY(), b = {
					...
					proxy = fly(document.body || document.documentElement)
							.createChild({
										style : {
											visbility : HIDDEN,
											position : ABSOLUTE,
		...
		fadeOut : function(o) {
			o = getObject(o);
			var me = this, dom = me.dom, style = dom.style, to = o.endOpacity
					|| 0;
			...
			me.queueFx(o, function() {
				arguments.callee.anim = fly(dom).fxanim({
							...
							if (to == 0) {
								Ext.Element.data(dom, 'visibilityMode') == Ext.Element.DISPLAY
										|| o.useDisplay
										? style.display = "none"
		...
		shift : function(o) {
			...
			this.queueFx(o, function() {
						...
						a.width
								? a.width.to = fly(dom).adjustWidth(o.width)
								: a;
						a.height
								? a.height.to = fly(dom).adjustWidth(o.height)
								: a;
						...
						arguments.callee.anim = fly(dom).fxanim(a, o, MOTION,
								.35, EASEOUT, function() {
									fly(dom).afterFx(o);
								});
					});
		...
		ghost : function(anchor, o) {
			...
			me.queueFx(o, function() {
						...
						pt.by = fly(dom).switchStatements(anchor.toLowerCase(),
								function(v1, v2) {
									return [v1, v2];
								}, {
									t : [0, -h],
		...
		fxanim : function(args, opt, animType, defaultDur, defaultEase, cb) {
			...
			var anim = Ext.lib.Anim[animType](this.dom, args,
					(opt.duration || defaultDur) || .35,
					(opt.easing || defaultEase) || EASEOUT, cb, this);
			opt.anim = anim;
			return anim;
		}
	};
	...
	// When included, Ext.Fx is automatically applied to Element so that all
	// basic
...
Ext.CompositeElementLite.prototype = {
	...
	invoke : function(fn, args) {
		var els = this.elements, el = this.el;
		Ext.each(els, function(e) {
					el.dom = e;
...
(function() {
	...
	function doFormUpload(o, ps, url) {
		var id = Ext.id(), doc = document, frame = doc.createElement('iframe'), form = Ext
				.getDom(o.form), hiddens = [], hd;
				...
				if (doc) {
					if (doc.body) {
						if (/textarea/i.test((firstChild = doc.body.firstChild
								|| {}).tagName)) { // json response wrapped in
													// textarea
	...
	Ext.extend(Ext.data.Connection, Ext.util.Observable, {
			...
			if (me.fireEvent(BEFOREREQUEST, me, o)) {
				if (o.el) {
					if (!Ext.isEmpty(o.indicatorText)) {
						me.indicatorText = '<div class="loading-indicator">'
								+ o.indicatorText + "</div>";
					}
					if (me.indicatorText) {
						Ext.getDom(o.el).innerHTML = me.indicatorText;
					}
					o.success = (Ext.isFunction(o.success)
							? o.success
							: function() {
				...
				p = Ext.urlEncode(me.extraParams, typeof p == 'object' ? Ext
								.urlEncode(p) : p);
				...
				if (form = Ext.getDom(o.form)) {
					url = url || form.action;
					if (o.isUpload
							|| /multipart\/form-data/i.test(form
				...
				method = o.method || me.method
						|| ((p || o.xmlData || o.jsonData) ? POST : GET);

				if (method === GET
						&& (me.disableCaching && o.disableCaching !== false)
				...
				return o.callback ? o.callback.apply(o.scope, [o, UNDEFINED,
								UNDEFINED]) : null;
			}
		},
		...
		isLoading : function(transId) {
			return transId
					? Ext.lib.Ajax.isCallInProgress(transId)
					: !!this.transId;
		},
...
Ext.util.JSON = new (function() {
	var useHasOwn = !!{}.hasOwnProperty, isNative = Ext.USE_NATIVE_JSON && JSON
