...
public class BmProcessor extends AbstractProcessor {
    private static Pattern dynamicPattern = Pattern.compile("\\$\\{.*\\}");
    ...
    protected void visitAttribute(Attribute a, IFile file, CompositeMap map, IDocument doc) {
        ...
        if (Util.isBMReference(type)) {
            ...
            try {
                resource = BMUtil.getBMResourceFromClassPath(file.getProject(), bm);
            } catch (ApplicationException e) {
                e.printStackTrace();
            }
            if (!(resource instanceof IFile)) {
                int line = map.getLocation().getStartLine();
                IRegion region = AbstractValidator.getValueRegion(doc, line - 1, name, bm);
                String msg = null;
                if (bm.length() == 0)
                    msg = name + " 不能为空";
                else
                    msg = name + " : " + bm + " , BM不存在";
                AuroraBuilder.addMarker(file, msg, line, region, IMarker.SEVERITY_ERROR, AuroraBuilder.UNDEFINED_BM);
            }
        }
    }
