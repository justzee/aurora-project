...
public class ScreenProcessor extends AbstractProcessor {
    ...
    public void visitAttribute(Attribute a, IFile file, CompositeMap map, IDocument doc) {
        if (isScreenReference(a.getAttributeType())) {
            ...
            IRegion vregion = AbstractValidator.getValueRegion(doc, line - 1, name, value);
            if (value.length() == 0) {
                String msg = name + " 不能为空";
                AuroraBuilder.addMarker(file, msg, line, vregion, IMarker.SEVERITY_ERROR,
                        AuroraBuilder.UNDEFINED_SCREEN);
                return;
            }
            if (!siPattern.matcher(value).matches()) {
                String msg = value + " 可能不是一个有效的值";
                AuroraBuilder.addMarker(file, msg, line, vregion, IMarker.SEVERITY_WARNING,
                        AuroraBuilder.UNDEFINED_SCREEN);
                return;
            }
            value = value.split("\\?")[0];
            IContainer webDir = Util.findWebInf(file).getParent();
            IPath path = new Path(value).makeRelativeTo(webDir.getFullPath()).makeAbsolute();
            ...
            vregion = AbstractValidator.getValueRegion(doc, line - 1, name, value);
            AuroraBuilder.addMarker(file, msg, line, vregion, IMarker.SEVERITY_ERROR, AuroraBuilder.UNDEFINED_SCREEN);
        }
    }

    private boolean isScreenReference(IType attributeType) {
        if (attributeType instanceof SimpleType) {
            return AbstractSearchService.screenReference.equals(((SimpleType) attributeType).getReferenceTypeQName());
        }
        return false;
    }
