...
public class ReferenceTypeFinder extends CompositeMapIteator {
	...
	private List getMatch(CompositeMap map) throws ApplicationException {
		...
		if (element != null) {
			...
			for (Iterator it = attrib_list.iterator(); it.hasNext();) {
				...
				if (referenceOf) {
					...
					if (filter != null) {
						found = filter.found(map, map.get(attrib.getName()));
					}
					if (found) {
						isMacth = true;
						matchs.add(attrib);
	...
	private boolean isReferenceType(IType attributeType) {
