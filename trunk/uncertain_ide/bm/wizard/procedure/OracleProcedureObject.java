...
public class OracleProcedureObject {
	...
	public CompositeMap toCompositeMap() throws ApplicationException{
		if(parameters == null)
			initParameters();
		String pre = "bm";
		CompositeMap model = new CompositeMap(pre,AuroraConstant.BMUri,"model");
		CompositeMap operations = new CompositeMap(pre,AuroraConstant.BMUri,"operations");
		model.addChild(operations);
		CompositeMap operation = new CompositeMap(pre,AuroraConstant.BMUri,"operation");
		operations.addChild(operation);
		operation.put("name", "execute");
		if(parameters.size()>0){
			CompositeMap bm_parameters = new CompositeMap(pre,AuroraConstant.BMUri,"parameters");
		...
		CompositeMap update_sql = new CompositeMap(pre,AuroraConstant.BMUri,"update-sql");
		StringBuffer sb = new StringBuffer("");
		...
		if(parameters.size()>0){
			...
			for(Iterator it = parameters.iterator();it.hasNext();){
				...
				sb.append(para.getName().toLowerCase()).append("=>").append("${").append(convertParameterForUse(para.getName())).append("}");
			}
			if(!firstRecord)
				sb.append(newLine(5)+")");
		}
		sb.append(";"+newLine(4));
		String text = newLine(4)+"begin"+newLine(5)+return_string+getFullObjectName()+sb.toString()+"end;";
	...
	public void initParameters() throws ApplicationException{
		...
		Connection connection = DBConnectionUtil.getDBConnection(ProjectUtil.getIProjectFromSelection());
		...
		try {
			...
			while (rs.next()) {
				...
				if((argument_name==null||"".equals(argument_name))&&rs.getInt(2)!=0)
					continue;
				if(argument_name == null ||"".equals(argument_name))
					argument_name = "return_value";
				parameters.add(new OracleParameter(argument_name, rs.getInt(2), rs.getString(3), rs.getString(4)));
			}
...
class OracleParameter {
	 ...
	 * @param inOut
	 */
	public OracleParameter(String name, int position, String plsType, String inOut) {
		super();
		this.name = name;
