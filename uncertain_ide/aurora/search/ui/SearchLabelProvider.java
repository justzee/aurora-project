...
import aurora.search.core.AbstractMatch;

public class SearchLabelProvider extends LabelProvider implements
	...
	private static final String line_number = "{0}:";
	private static final String count_format = "({0} matches)";
	...
	public SearchLabelProvider(AbstractTextSearchViewPage page, int orderFlag) {
		fLabelProvider = new WorkbenchLabelProvider();
		fOrder = orderFlag;
		fPage = page;
		fLineMatchImage = SearchImages.getImage(SearchImages.LINE_MATCH);
		fMatchComparator = new Comparator() {
			public int compare(Object o1, Object o2) {
				return ((AbstractMatch) o1).getOriginalOffset()
						- ((AbstractMatch) o2).getOriginalOffset();
			}
		};
	}
	...
	private StyledString getLineElementLabel(LineElement lineElement) {
		...
		int charsToCut = getCharsToCut(length, matches); // number of characters
															// to leave away if
															// the line is too
															// long
		for (int i = 0; i < matches.length; i++) {
			AbstractMatch match = (AbstractMatch) matches[i];
			int start = Math.max(
					match.getOriginalOffset() - lineElement.getOffset(), 0);
			// append gap between last match and the new one
			if (pos < start) {
				if (charsToCut > 0) {
					charsToCut = appendShortenedGap(content, pos, start,
							charsToCut, i == 0, str);
				} else {
			...
			// append match
			int end = Math.min(
					match.getOriginalOffset() + match.getOriginalLength()
							- lineElement.getOffset(), lineElement.getLength());
			str.append(content.substring(start, end),
					DecoratingSearchLabelProvider.HIGHLIGHT_STYLE);
			pos = end;
		}
	...
	private int appendShortenedGap(String content, int start, int end,
			int charsToCut, boolean isFirst, StyledString str) {
	...
	private int getCharsToCut(int contentLength, Match[] matches) {
		if (contentLength <= 256
				|| !"win32".equals(SWT.getPlatform()) || matches.length == 0) { //$NON-NLS-1$
			return 0; // no shortening required
		}
		// XXX: workaround for
		// https://bugs.eclipse.org/bugs/show_bug.cgi?id=38519
		return contentLength - 256
				+ Math.max(matches.length * fgEllipses.length(), 100);
	}

	private int evaluateLineStart(Match[] matches, String lineContent,
			int lineOffset) {
		int max = lineContent.length();
		if (matches.length > 0) {
			AbstractMatch match = (AbstractMatch) matches[0];
