 ...
 */
public class JavaScriptAutoIndentStrategy extends DefaultIndentLineAutoEditStrategy {

    public JavaScriptAutoIndentStrategy() {
    }
    ...
    protected void smartIndentAfterNewLine(IDocument document, DocumentCommand command) {
        ...
        try {
            ...
            if (command.offset < docLength && document.getChar(command.offset) == '}') {
                ...
                String indent = getIndentOfLine(document, indLine);
                // buf.append("    " + indent + "\r\n" + indent);
                document.replace(command.offset, 1, "\r\n" + indent + XMLOutputter.DEFAULT_INDENT + "\r\n" + indent
                        + "}");
                command.caretOffset = command.offset + ("\r\n" + indent + XMLOutputter.DEFAULT_INDENT).length();
                command.text = "";
                return;
            } else {
                int start = document.getLineOffset(line);
