...
public class ForeignFieldProcessor extends AbstractProcessor {
    ...
    public void visitAttribute(Attribute a, IFile file, CompositeMap map, IDocument doc) {
        ...
        try {
            bmfile = BMUtil.getBMResourceFromClassPath(file.getProject(), refModel);
        ...
        if (bmfile instanceof IFile) {
            ...
            try {
                bmmap = CacheManager.getCompositeMapCacher().getCompositeMap((IFile) bmfile);
            } catch (CoreException e) {
                e.printStackTrace();
            } catch (ApplicationException e) {
                e.printStackTrace();
            }
            ReferenceTypeFinder rtf = new ReferenceTypeFinder(AbstractSearchService.localFieldReference);
            ...
            // 未在参照bm中找到field
            int sevrity = "screen".equals(file.getFileExtension()) ? IMarker.SEVERITY_WARNING : IMarker.SEVERITY_ERROR;
            AuroraBuilder.addMarker(file, a.getName() + " : " + value + " 没有在参照BM : " + refModel + " 中定义", line,
                    region, sevrity, AuroraBuilder.UNDEFINED_FOREIGNFIELD);
        } else {
            // 参照BM不存在,此处不作处理
        }
    }
    ...
    private boolean isForeignFieldReference(IType attributeType) {
        if (attributeType instanceof SimpleType) {
            return AbstractSearchService.foreignFieldReference.equals(((SimpleType) attributeType)
