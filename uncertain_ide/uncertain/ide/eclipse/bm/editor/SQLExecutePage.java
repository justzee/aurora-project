...
import uncertain.ide.eclipse.bm.editor.toolbar.action.ExecuteSqlAction;
...
public class SQLExecutePage extends FormPage implements ISqlViewer {
	...
	public void refresh(ResultSet resultSet, int resultCount) {
		if (resultSet != null) {
			...
			int message = CustomDialog.showConfirmDialogBox(null, LocaleMessage
	...
	public void refresh(String data) throws ApplicationException{
		...
		for (int i = 0; i < tabs.length; i++) {
			...
			try {
				...
				st.setText(CustomDialog.getExceptionMessage(e));
			}
		}
	...
	protected void initConnection() throws ApplicationException {
		...
		IDEModelFactory modelFactory = new IDEModelFactory(uncertainEngine
				.getOcManager());
		...
		CompositeMap bm_model;
		try {
			bm_model = svcFactory.getModelFactory().getCompositeLoader()
					.loadByFullFilePath(getFile().getAbsolutePath());
			service = svcFactory.getModelService(bm_model, context);
		} catch (IOException e) {
			throw new ApplicationException("文件路径错误!", e);
		} catch (SAXException e) {
			throw new ApplicationException("文件解析有误!", e);
		}
	}
		...
		for (int i = 0; i < tabs.length; i++) {
			...
			tabFolder.addSelectionListener(new SelectionListener() {
				...
				public void widgetSelected(SelectionEvent e) {
					...
					try {
						StringBuffer sqlbf = service.getSql(tabs[itemIndex]);
						String sql = sqlbf.toString();
						if (sqlbf != null){
							st.setText(sql);
						}
					} catch (Throwable ex) {
						st.setText(CustomDialog.getExceptionMessage(ex));
					}
				}
			});
		}
