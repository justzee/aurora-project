package uncertain.ide.eclipse.screen.wizard;
...
public class NewScreenTemplatesWizardPage extends WizardPage {
	...
	public NewScreenTemplatesWizardPage() {
		super(
				"NewScreenTemplatesWizardPage", LocaleMessage.getString("screen.wizard"), null); //$NON-NLS-1$
		setDescription(LocaleMessage
				.getString("select.template.as.the.new.screen.content"));
	}
	...
	private StyledText createViewer(Composite parent) {
		StyledText viewer = new StyledText(parent, SWT.BORDER | SWT.V_SCROLL
				| SWT.H_SCROLL);
		return viewer;
	}
	...
	private String getPreferencePageId() {
		return "uncertain.ide.eclipse.preferencePage.TemplatesPage"; //$NON-NLS-1$
	}
	...
	String getTemplateString() {
		...
		if (template != null) {
			TemplateContextType contextType = AuroraTemplateManager
					.getInstance().getContextTypeRegistry().getContextType(
	...
	void updateViewerInput() {
		Template template = getSelectedTemplate();
		if (template != null) {
			templateContent = template.getPattern();
