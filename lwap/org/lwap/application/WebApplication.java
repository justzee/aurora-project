...
public static final String APPLICATION_CONFIG_PATH = "application.xml";
  ...
  Service _create_service( CompositeMap service_conf) throws ServiceInstantiationException {
  	 	String service_class = service_conf.getString(KEY_CLASS);  	 	
  	 	/*
  	 	if( service_class == null){ 

  	 	    System.out.print(service_conf.getName());
  	 	    System.out.println(service_conf.entrySet());
  	 	    Iterator it = service_conf.getChildIterator();
  	 	    if(it!=null)
  	 	        while(it.hasNext()) {
  	 	    ...
  	 	    //System.out.println(service_conf.toXML());
	 	    throw new ServiceInstantiationException("No class specified in service config");
  	 	}
  	 	*/
  	 	/*
        if(service_class==null)
  ...
  */
  ...
  Service _create_service( String service_name) throws ServiceInstantiationException{
		...
		try{
			...
			throw new ServiceInstantiationException("XML Syntax error in service config", sex);
		} 
  	 	
  	 	if( service_conf == null) throw new ServiceInstantiationException("Can't load service config:"+service_name);
  	 	Service service_instance = _create_service(service_conf);
  	 	service_instance.setServiceName(service_name);
  	 	return service_instance;
  }

  void init_participant_list( CompositeMap svc_list, Collection list ) throws Exception {
  		if( svc_list == null) return;
  		Iterator it = svc_list.getChildIterator();
  		if( it == null) return;
  		while( it.hasNext()){
  			CompositeMap svc_def = (CompositeMap)it.next();
  			ServiceParticipant svc_instance = (ServiceParticipant) Class.forName(svc_def.getString(KEY_CLASS)).newInstance();
...
/* ------------------ BEGIN get/set application scope objects -----------------*/
  ...
  public void setDataSource(DataSource data_source) {
        ...
        transaction_factory = new TransactionFactory(data_source, composite_loader);
  }
...
/* ------------------ END get/set application scope objects -----------------*/
  ...
  public String getLocalizedString( Locale locale,  String input){
  	if( getResourceBundleFactory() == null || input == null) return input;
  	ResourceBundle bundle = getResourceBundleFactory().getResourceBundle(locale);
  	return bundle == null?input : bundle.getString(input);
  }
  
  public Service getService( CompositeMap service_config) throws ServiceInstantiationException {
  	    Service service_instance = _create_service(service_config);
  	    return service_instance;
  }  
  

  public Service getService(String service_name, CompositeMap context) throws ServiceInstantiationException {
   ...
   *  @return object if instance is successfuly created 
   *           else return null
   */
  ...
  /** called by FacadeService to perform per-service works such as session check */
  public int initService( HttpServletRequest request, HttpServletResponse response, Service service ) throws IOException, ServletException {
     int ret = 0;
  	 ...
  	 while( it.hasNext()){
  	 	...
  	 	if( ret == ServiceParticipant.BREAK_PRE_SERVICE_LIST) break;
  	 	else if( ret == ServiceParticipant.BREAK_WHOLE_SERVICE) return ret;
  	 }  
     return ret;
  }
  ...
  public ServiceDispatch createUrlDispatch(BaseService origin_service, String url, int dispatch_style) {
  	return new ServiceDispatch(origin_service, ServiceDispatch.TARGET_TYPE_URL, url, dispatch_style);
  }
  
  public ServiceDispatch createServiceDispatch(BaseService origin_service, String service_name, int dispatch_style){
  	return new ServiceDispatch(origin_service, ServiceDispatch.TARGET_TYPE_SERVICE, service_name, dispatch_style);  	
  }
  
  public ServiceDispatch createDispatch( BaseService service, int target_type, String target_name, int dispatch_style){
  	return new ServiceDispatch(service, target_type, target_name, dispatch_style);  	
  }
  	...
  	if( cfg != null){
  		...
  		if( it != null){
  			...
  			while( it.hasNext()){
				...
				File pathFile = null;
  				if( path != null){
  				        pathFile = new File(getCompositeLoader().getBaseDir(), path);
  				}  
  				else{
  ...
  void loadFormat(CompositeMap config){
  		if( config.containsKey(KEY_DATE_FORMAT)){
  			DataTypeManager.dateFormat = new SimpleDateFormat(config.getString(KEY_DATE_FORMAT));
  		}

  		if( config.containsKey(KEY_DATETIME_FORMAT)){
  			DataTypeManager.datetimeFormat = new SimpleDateFormat(config.getString(KEY_DATETIME_FORMAT));
  		}

  		if( config.containsKey(KEY_TIME_FORMAT)){
  			DataTypeManager.time_format = new SimpleDateFormat(config.getString(KEY_TIME_FORMAT));
  		}  
  }
  ...
  public void init( CompositeMap config) throws ApplicationInitializeException {
        ...
        serviceListenerManager = new BasicServiceListenerManager();
  		...
  		while( it.hasNext()){
  			...
  			if( cls != null)
  			try{
  				ApplicationInitializer a = (ApplicationInitializer)Class.forName(cls).newInstance();
        ...
        if(isRecordRerformance()){
            performance_recorder.setRecordDetail(config.getBoolean(KEY_PERFORMANCE_DETAIL, false));
        }  		
  ...
  void load_exception_handle(CompositeMap config){
  	...
  	while( it.hasNext()){
  		...
  		if( exception != null && cls != null){
  			ExceptionHandle handle = (ExceptionHandle)this.getPooledObject(cls);
  			if( handle != null){
  				item.put(KEY_CLASS, handle); 
  				exception_handle_map.put(exception, handle);
  				//exception_handle_config.put(exception, item);
  			}
  			else System.err.println(this.getClass().getName() + ": can't load exception handle " + cls );
  		}
  	}
  }
  ...
  public WebApplication( String doc_path, String config_file_name, ServletContext context) 
  throws ApplicationInitializeException{
  	

  	this.app_path = doc_path;
  	this.servlet_context = context;
  	if( config_file_name == null) config_file_name = APPLICATION_CONFIG_PATH;
  	...
  	try{
  		...
  		throw new ApplicationInitializeException("IOException while loading config file:"+ioex.getMessage());
  	} catch( org.xml.sax.SAXException sex){
  		throw new ApplicationInitializeException("Config file syntax error:"+sex.getMessage());
  	}
  ...
  public ExceptionHandle getExceptionHandle( CompositeMap handle_config){
  	if( handle_config == null) return null;
  	ExceptionHandle handle = (ExceptionHandle)handle_config.get(KEY_CLASS);
 	if( handle == null) handle = default_exception_handle;
 	return handle;
  }
  
  
  public void handleException( Service svc, Throwable thr){
  	if( thr == null) throw new NullPointerException();
  	Throwable cause = thr.getCause();
  	if( cause != null) thr = cause;
  	
  	ExceptionHandle handle = null;
  	CompositeMap config = getExceptionHandleConfig( thr );
  	if( config == null) handle = default_exception_handle;
  	else handle = getExceptionHandle(config);
    ...
    /**
     * @param record_performance the record_performance to set
     */
    public void setRecordRerformance(boolean record_performance) {
        this.record_performance = record_performance;
        if(!record_performance){
            if(performance_recorder!=null) performance_recorder.clear();
